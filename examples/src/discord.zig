...
pub fn main(init: std.process.Init) !void {
    ...
    var oauth2_provider = try DiscordProvider.init(io, allocator, .{
    ...
    var app = App{ .io = io, .discord = &oauth2_provider, .session_store = &session_store };
...
fn handleCallback(app: *App, req: *httpz.Request, res: *httpz.Response) !void {
    ...
    var tokens = try app.discord.validateAuthorizationCode(res.arena, code, &.{});
    defer tokens.deinit();

    const user_profile = try getUserProfile(app.io, res.arena, "https://discord.com/api/users/@me", tokens.parsed.?.value.access_token);

...
pub fn main(init: std.process.Init) !void {
    ...
    var oauth2_provider = try LinkedInProvider.init(io, allocator, .{
    ...
    var app = App{ .io = io, .linkedin = &oauth2_provider, .session_store = &session_store };
...
fn handleCallback(app: *App, req: *httpz.Request, res: *httpz.Response) !void {
    ...
    var tokens = try app.linkedin.validateAuthorizationCode(res.arena, code, &.{});
    defer tokens.deinit();

    const user_profile = try getUserProfile(app.io, res.arena, "https://api.linkedin.com/v2/userinfo", tokens.parsed.?.value.access_token);
